...
namespace MovieTicketManagementSystem.PresentationLayer
{
    public partial class buyTicketForm : UserControl
    {
        ...
        public void displayAvailableMovies()
        {
            ...
            List<MovieDTO> listData = mData.movieAvailableListData();
        ...
        private void buyTicket_calculateBtn_Click(object sender, EventArgs e)
        {
            using(SqlConnection connect = new SqlConnection(conn))
            {
                ...
                buyTicket_totalPrice.Text = "VND" + getTotal.ToString("0.000");
            }
        }
            ...
            if(e.KeyCode == Keys.Enter)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        buyTicket_change.Text = "VND" + getChange.ToString("0.000");
                    }
                    catch (Exception ex)
                    {
        ...
        private void buyTicket_buyBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (getAmount == 0 && !string.IsNullOrWhiteSpace(buyTicket_amount.Text))
                {
                    try
                    {
                        double inputAmount = Convert.ToDouble(buyTicket_amount.Text);
                        ...
                        {
                            MessageBox.Show("Error: Amount should be greater than total", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                    }
                    catch
                    {
                        MessageBox.Show("Please enter valid number in amount field", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                }
                ...
                using (SqlConnection connect = new SqlConnection(conn))
                {
                    string insertCustomer = "INSERT INTO customers (movie_id, seat_type, available_seat, totalPrice, foods, drinks, amount, change, status, date_buy) " +
                        "VALUES (@movieID, @seatType, @availableSeat, @totalPrice, @foods, @drinks, @amount, @change, @status, @dateBuy)";
                    using (SqlCommand cmd = new SqlCommand(insertCustomer, connect))
                    {
                        cmd.Parameters.AddWithValue("@movieID", movie_id);
                        cmd.Parameters.AddWithValue("@seatType", "Regular"); 
                        cmd.Parameters.AddWithValue("@availableSeat", 1); 
                        cmd.Parameters.AddWithValue("@totalPrice", getTotal);
                        cmd.Parameters.AddWithValue("@foods", buyTicket_foods.Text);
                        cmd.Parameters.AddWithValue("@drinks", buyTicket_drinks.Text);
                    ...
                    using (SqlCommand cmd = new SqlCommand(insertData, connect))
                    {
                        ...
                        MessageBox.Show($"Successful! occupied: {buyTicket_availableChairs.Text}", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
        }
        ...
        public void clearFields()
        {
            ...
            buyTicket_totalPrice.Text = "VND 0.000";
            buyTicket_amount.Text = "";
            buyTicket_change.Text = "VND 0.000";
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "KIET DUYEN PHUC Cinema";
            ...
            while(rowIndex < dataGridView1.Rows.Count)
            {
                ...
                e.Graphics.DrawString("Total Price: \tVND " + getTotal.ToString("0.000") + "\nAmount: \tVND " + buyTicket_amount.Text
        ...
        private void buyTicket_change_Click(object sender, EventArgs e)
        {
